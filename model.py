...
from FCoTNetBlock import FCoTNetLayer
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.fcot_layer(out)  # FCoTNetLayer
        out = self.bn2(out)
        out = self.relu(out)
...
class CoTResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=3):
        ...
        self.fc0 = nn.Linear(512 * block.expansion, 512 * block.expansion)
...
def ResidualNet(Bottleneck,network_type, depth, num_classes, att_type=None):
    ...
    if depth == 18:
        model = CoTResNet(BasicBlock, [2, 2, 2, 2], num_classes)
    elif depth == 34:
        model = CoTResNet(BasicBlock, [3, 4, 6, 3], num_classes)
    elif depth == 50:
        model = CoTResNet(Bottleneck,[3, 4, 6, 3], num_classes)
    elif depth == 101:
        model = CoTResNet(Bottleneck, [3, 4, 23, 3], num_classes)
